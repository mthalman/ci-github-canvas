...
// Serializes disk writes. Without this, two overlapping persist() calls (e.g.
// concurrent POSTs from two canvas instances) each kick off an independent
// writeFile, and an older write can land after a newer one — clobbering
// settings.json with stale contents. Chaining every write behind the previous
// one guarantees they apply in call order.
...
function persist() {
    // Snapshot the document synchronously so this write commits exactly the
    // state intended at call time, then queue it behind any in-flight write.
    const snapshot = JSON.stringify(settingsDoc, null, 2);
    persistChain = persistChain.then(async () => {
        try {
            // The artifacts dir doesn't exist on a fresh install; the first
