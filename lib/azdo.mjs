...
export function parseAzdoBuildUrl(input) {
    ...
    const idMatch = trimmed.match(AZDO_BUILD_ID_RE);
    if (!idMatch) return null;
    return { org, project, buildId: idMatch[1] };
}
...
export async function fetchAzdoBuild({ org, project, buildId, force = false, runAz = defaultRunAz }) {
    const key = `${org}|${project}|${buildId}`;
    const now = Date.now();
    if (!force) {
    ...
    const promise = (async () => {
        ...
        const store = (result) => {
            azdoBuildCache.set(key, { at: now, ...result });
            return { cachedAt: now, ...result };
        };
        // 1. Try anonymous first — public pipelines need no auth.
        try {
            const value = await fetchBoth();
            return store({ value, data: value, error: null, errorKind: null, auth: "anonymous" });
        } catch (anonErr) {
            if (!anonErr.authRequired) {
                return store({ value: null, data: null, error: anonErr.message, errorKind: "fetch_failed", auth: "anonymous" });
            }
            // 2. Anonymous was refused — fall back to Azure CLI auth.
            const tok = await getAzdoAccessToken({ runAz });
            if (tok.error) {
                return store({ value: null, data: null, error: tok.error, errorKind: tok.errorKind, auth: "azure-cli" });
            }
            try {
                const value = await fetchBoth({ token: tok.token });
                return store({ value, data: value, error: null, errorKind: null, auth: "azure-cli" });
            } catch (authErr) {
                ...
                const message = authErr.authRequired
                    ...
                    : authErr.message;
                return store({ value: null, data: null, error: message, errorKind, auth: "azure-cli" });
            }
        } finally {
