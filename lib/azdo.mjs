...
export function parseAzdoBuildUrl(input) {
    ...
    // Require the documented build-results path so unrelated AzDO URLs that
    // merely carry a buildId query param (e.g. a release or wiki URL) are
    // rejected, matching this function's "build results URL" contract.
    let pathname;
    try {
    ...
    if (!/\/_build\/results\/?$/i.test(pathname)) return null;
...
export async function fetchAzdoTimeline({ org, project, buildId, force = false }) {
    ...
    const promise = (async () => {
        ...
        try {
            ...
            // Only clear the in-flight entry if it still points at this promise.
            // A concurrent force=true refresh may have replaced it with a newer
            // promise; deleting unconditionally would defeat de-dupe.
            if (azdoTimelineInFlight.get(key) === promise) {
...
export async function fetchAzdoBuild({ org, project, buildId, force = false, runAz = defaultRunAz, getAccessToken = null }) {
    ...
    const promise = (async () => {
        ...
        try {
            ...
            // A concurrent force=true refresh may have replaced it with a newer
            // promise; deleting unconditionally would defeat de-dupe.
            if (azdoBuildInFlight.get(key) === promise) {
