...
import { fetchAzdoBuild, fetchAzdoTimeline, parseAzdoBuildUrl, createSharedTokenGetter } from "./azdo.mjs";
import { enrichSessionsWithSyncState } from "./git-sync.mjs";
import { displayConfig, saveDisplayConfig } from "./display.mjs";
import { fetchAgentTasks, fetchAuthoredPrs, fetchChecksForRefs, fetchPrsWithChecks } from "./github.mjs";
...
import { fetchCopilotSessions, filterSessionsByLivePrState, sessionPrRefs } from "./sessions.mjs";
import { loadSnapshot, saveSnapshot, snapshotInlineScript } from "./snapshot.mjs";
import {
    addWatchedPr,
...
export async function startServer({ ciRunUrl = null, instanceId = null } = {}) {
    ...
    const server = createServer(async (req, res) => {
        try {
            ...
            if (url.pathname === "/") {
                ...
                let html = configuredCiRunUrls.length > 0
                    ? PAGE_HTML.replace("<body>", '<body class="inspect-mode">')
                    : PAGE_HTML;
                // Inline the last-rendered snapshot (SSR) so the page paints
                // prior content before any fetch — surviving the ephemeral-port
                // change that wipes the client's per-origin localStorage when the
                // webview/host is recreated. The client's loads then reconcile it
                // inline. A function replacement avoids "$"-sequence
                // interpretation of the inlined JSON.
                const inline = snapshotInlineScript(await loadSnapshot(instanceId));
                if (inline) html = html.replace(/<body[^>]*>/, (m) => m + inline);
                res.end(html);
                ...
                jsonResponse(res, { rows: data ?? [], cachedAt, error });
            } else if (url.pathname === "/api/session-checks") {
                // CI run trees for the PRs backing the local Copilot sessions —
                // including ones NOT authored by @me (which /api/prs-with-checks
                // misses). The Copilot tab uses this to show run trees for
                // others' session PRs when "Show others' PRs" is enabled.
                const force = url.searchParams.get("force") === "1";
                const rows = fetchCopilotSessions();
                if (rows && rows.__error) {
                    jsonResponse(res, { rows: [], error: rows.__error });
                } else {
                    const refs = [];
                    const seen = new Set();
                    for (const row of Array.isArray(rows) ? rows : []) {
                        for (const ref of sessionPrRefs(row)) {
                            if (seen.has(ref.key)) continue;
                            seen.add(ref.key);
                            refs.push(ref);
                        }
                    }
                    const { data, cachedAt, error } = await fetchChecksForRefs(refs, { force });
                    jsonResponse(res, { rows: data ?? [], cachedAt, error });
                }
                ...
                if (configuredCiRunUrls.length === 0) {
                    ...
                    const force = url.searchParams.get("force") === "1";
                    // Share one Azure CLI token across all private runs in this
                    // refresh so `az` is spawned at most once, not once per run.
                    const getAccessToken = createSharedTokenGetter();
                    const runs = await Promise.all(configuredCiRunUrls.map(async (runUrl) => {
                        ...
                        const { data, cachedAt, error, errorKind, auth } = await fetchAzdoBuild({ org, project, buildId, force, getAccessToken });
                        return {
                            url: runUrl,
                            org,
                            project,
            ...
            } else if (url.pathname === "/api/display-config") {
                if (req.method === "GET") {
                    jsonResponse(res, { config: displayConfig });
                } else if (req.method === "POST") {
                    try {
                        const body = await readJsonBody(req);
                        const updated = await saveDisplayConfig(body);
                        jsonResponse(res, { config: updated });
                    } catch (err) {
                        jsonResponse(res, { error: String(err?.message ?? err) }, 400);
                    }
                ...
                // The canvas page POSTs its last-rendered panels here so the
                // next page load (after a webview/host recreation on a new port)
                // can inline them via SSR. Display cache only — never read by any
                // other consumer.
                if (req.method === "POST") {
                    try {
                        const body = await readJsonBody(req);
                        await saveSnapshot(instanceId, body);
    ...
    // How many runs the panel currently holds — lets the canvas decide whether
    // it's in inspect mode regardless of whether the current open() call passed
    // a ciRunUrl (a re-open/focus without one must not flip the status away).
    const ciRunCount = () => configuredCiRunUrls.length;
    return { server, url: `http://127.0.0.1:${port}/`, setCiRunUrl, addCiRunUrl, removeCiRunUrl, ciRunCount };
}
