...
// a runaway payload can't bloat the snapshot file or the served page. Kept well
// under a quarter of readJsonBody's 1MB cap in server.mjs so that even a full
// snapshot of all four panels (plus JSON escaping overhead) can't structurally
// exceed the POST body limit and get silently rejected.
...
export function sanitizeSnapshot(raw) {
    ...
    if (raw.html && typeof raw.html === "object" && !Array.isArray(raw.html)) {
        ...
        for (const [id, val] of Object.entries(raw.html)) {
            ...
            if (typeof val !== "string" || val.length > MAX_HTML_BYTES) continue;
            html[id] = val;
        }
