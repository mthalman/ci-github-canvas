...
// Callback supplied by extension.mjs that reports whether THIS session
// currently has the CI Runs canvas open. The Copilot app runs one process per
// session, so each process's notifier only owns its own session's handle
// (`activeSession`). Without this gate every session that loaded the extension
// would fire a session.send() on every poll — including sessions where the
// user never opened the canvas. We only want alerts to land in the session(s)
// actively watching the canvas, so the notifier consults this provider and
// stays silent when the panel isn't open here. The provider may return a count
// (number of open panels) or a boolean; either is coerced to open/closed.
let canvasOpenProvider = null;
export function setCanvasOpenProvider(fn) {
    canvasOpenProvider = typeof fn === "function" ? fn : null;
}

// True when this session has at least one CI Runs canvas panel open. Defaults
// to closed when no provider is wired (e.g. in unit tests that never call
// setCanvasOpenProvider), so the notifier fails safe to "don't send".
export function isCanvasOpen() {
    if (typeof canvasOpenProvider !== "function") return false;
    try {
        const v = canvasOpenProvider();
        return typeof v === "number" ? v > 0 : Boolean(v);
    } catch (err) {
        console.error("ci-runs: canvas-open provider threw", err);
        return false;
    }
}

// Count canvas panels open in PR-dashboard mode — i.e. NOT inspect mode. The
// notifier watches the user's authored/watched PRs, and inspect mode (a panel
// opened with one or more `ciRunUrl`s) hides the PR tabs entirely to focus on
// those standalone Azure DevOps runs. So a panel only justifies a PR alert
// while it's showing the dashboard; an inspect-only session must stay silent
// for PR transitions that have nothing to do with the runs it's inspecting.
//
// `servers` is extension.mjs's instanceId→entry map; an entry is in inspect
// mode when it currently holds at least one ciRunUrl (ciRunCount() > 0). A
// panel that started on the dashboard and later had a run added flips to
// inspect mode here too, matching the served HTML hiding its PR tabs.
export function countDashboardPanels(servers) {
    let n = 0;
...
export async function runNotifyPoll() {
    ...
    try {
        ...
        // Only this session's process should alert if the canvas is open here.
        // State maps above are still refreshed every poll, so transitions that
        // happen while the panel is closed are absorbed silently — reopening
        // the canvas won't replay a backlog of stale completions/failures.
