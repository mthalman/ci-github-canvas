...
<body>
  ...
  <script>
    ...
    function reconcileRows(container, items, opts) {
      ...
      // Drop the initial "Loading…" placeholder the first time we render real
      // content. It comes in two shapes: an element child with class "loading"
      // (the copilot/all/cirun panels), or — for #watched-list, which is itself
      // the reconciler container — the container's own "loading" class plus a
      // bare "Loading…" text-node child. The old whole-innerHTML path wiped
      // either implicitly; the reconciler keeps the container and only manages
      // its own list/banner/empty nodes, so it must clear both forms explicitly
      // or they linger above the rows. (childNodes, not children, so stray text
      // nodes are caught too.)
      ...
      for (const child of Array.from(container.childNodes)) {
        ...
        if (child.nodeType === 1 && child.classList && child.classList.contains('loading')) child.remove();
      }
