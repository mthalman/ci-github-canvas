...
<body>
  ...
  <script>
    ...
    function reconcileRows(container, items, opts) {
      ...
      //   - snapshot hydration, which injects the last render as raw innerHTML,
      //     so an empty/error state lands as a direct-child <div class="empty">
      //     / <div class="error">. The reconciler's managed empty/banner nodes
      //     are WRAPPER divs (their .empty/.error lives one level down), so a
      //     direct-child .empty/.error is always a stale hydrate artifact —
      //     left in place it would sit above the rows, or duplicate when the
      //     branch below builds a fresh managed node.
      ...
      for (const child of Array.from(container.childNodes)) {
        ...
        if (child.classList.contains('loading') || child.classList.contains('empty') || child.classList.contains('error')) {
          child.remove();
        }
      }
