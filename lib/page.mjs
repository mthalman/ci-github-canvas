    ...
    async function loadCiRun(force = false) {
      ...
      try {
        ...
        if (stamped || inspectMode) {
          ...
          // Self-heal faster than the 60s poll: schedule a short follow-up
          // retry so a service that comes back a few seconds later clears the
          // error box on its own. If that retry also fails it re-enters this
          // branch and re-schedules, so the panel keeps trying (~every few
          // seconds) for as long as the service stays down instead of waiting
          // on the 60s poll. Guarded so overlapping failures don't stack timers
          // and a successful load (below) cancels any pending retry.
          if (__cirunRetryTimer === null) {
            __cirunRetryTimer = setTimeout(async () => {
              __cirunRetryTimer = null;
              // Respect the same overlap guard autoRefresh() uses so a self-heal
              // retry can't run concurrently with an in-flight auto-poll refresh
              // (which would cause redundant requests and out-of-order
              // reconciles). If a refresh is already running it will re-render
              // the panel — and reschedule its own self-heal on failure — so we
              // simply skip this tick. The try/catch mirrors autoRefresh too, so
              // an unexpected throw can't surface as an unhandled rejection.
              if (refreshing) return;
              refreshing = true;
              try { await loadCiRun(true); }
              catch (e) { console.error('ci-run self-heal retry failed', e); }
