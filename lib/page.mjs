...
<body>
  ...
  <script>
    ...
    function reconcileRows(container, items, opts) {
      ...
      // Drop the server-rendered "Loading…" placeholder (or any other stray,
      // non-managed child) the first time we render real content. The old
      // whole-innerHTML path replaced it implicitly; the reconciler keeps the
      // container and only manages its own list/banner/empty nodes, so it must
      // remove the placeholder explicitly or it lingers above the rows.
      for (const child of Array.from(container.children)) {
        if (child === container.__listEl || child === container.__bannerEl || child === container.__emptyEl) continue;
        if (child.classList && child.classList.contains('loading')) child.remove();
      }
