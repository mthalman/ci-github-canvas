  ...
  <script>
    ...
    async function loadCopilot() {
      ...
      if (res.error) {
        ...
        // very thing this snapshot exists to prevent.
        const banner = \`<div class="error">\${esc(res.error)}</div>\`;
        reconcileRows(document.getElementById('panel-copilot'), lastCopilotItems,
          { emptyHtml: renderCopilot([]).emptyHtml, errorHtml: banner });
