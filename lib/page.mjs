    ...
    <div class="menu-anchor">
      ...
      <div class="menu" id="settings-menu" role="menu" aria-label="CI Runs settings" hidden>
        ...
        <div class="menu-section" role="group" aria-label="Repository filter">
          ...
          <p class="menu-hint">Glob patterns matched against <code>owner/repo</code>, one per line. <code>*</code> matches any characters, <code>?</code> one; prefix with <code>!</code> to exclude. Last matching line wins (like <code>.gitignore</code>), so a later line can override an earlier one. Empty = all repos.</p>
  ...
  <script>
    ...
    function renderCiRunCard(run) {
      ...
      if (run.error) {
        ...
        return \`<li class="row cirun-error-row">
      ...
      // (snapshot/restore in loadCiRun). The outer row and the inner job list
      // each get their own key. Include project because AzDO build IDs are only
      // unique within a project, so two runs in the same org but different
      // projects could otherwise collide and swap each other's open/closed state.
      ...
      return \`<li class="row row-collapsible"><details data-jobs-key="\${rowKey}" open>
        <summary>
          <span class="caret">▶</span>
    ...
    function renderCiRun(res) {
      const runs = res.runs || [];
      if (runs.length === 0) return '<div class="empty">No CI run configured.</div>';
      return \`<ul class="list cirun-wrap">\${runs.map(renderCiRunCard).join('')}</ul>\`;
    }
    ...
    async function loadCiRun(force = false) {
      ...
      let res;
      try {
        res = await fetch('/api/ci-run' + (force ? '?force=1' : '')).then(r => r.json());
      } catch (e) {
        // Network/parse failure. In inspect mode the run panel is the only
        // visible surface (the PR tabs are hidden by CSS), so falling through
        // would strand the user on the panel's "Loading…" placeholder forever.
        // Replace it with an actionable, auto-retrying error instead.
      ...
      // Preserve each row/build's manual collapse/expand across the re-render.
      const openState = new Map();
      panel.querySelectorAll('details[data-jobs-key]').forEach(d => openState.set(d.dataset.jobsKey, d.open));
      panel.innerHTML = renderCiRun(res);
      panel.querySelectorAll('details[data-jobs-key]').forEach(d => {
        if (openState.has(d.dataset.jobsKey)) d.open = openState.get(d.dataset.jobsKey);
      });
      return true;
    }
    ...
    async function reloadSettings() {
      ...
      try {
        await Promise.all([loadNotifyConfig(), loadRepoFilter()]);
        if (!dirty) setStatus('');
      } finally {
