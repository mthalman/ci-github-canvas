...
<head>
  ...
  <style>
    ...
    :root {
      ...
      --accent-emphasis: var(--true-color-blue, #1f6feb);
...
<body>
      ...
      <div class="menu" id="settings-menu" role="menu" aria-label="CI Runs settings" hidden>
        ...
        <div class="menu-section" role="group" aria-label="Repository filter">
          ...
          <div class="menu-actions">
            <span class="menu-save-status" id="repo-filter-status" aria-live="polite"></span>
            <button type="button" class="menu-save-btn" id="repo-filter-save">Save</button>
          </div>
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    // ---- Repo filter (single glob list; '!' prefix excludes) ----
    const repoPatternsEl = document.getElementById('repo-patterns');
    const repoSaveBtn = document.getElementById('repo-filter-save');
    const repoStatusEl = document.getElementById('repo-filter-status');
    ...
    function setRepoStatus(msg) {
      repoStatusEl.textContent = msg;
      if (repoStatusTimer) clearTimeout(repoStatusTimer);
      if (msg) repoStatusTimer = setTimeout(() => { repoStatusEl.textContent = ''; }, 2500);
    }
    ...
    async function saveRepoFilter() {
      const payload = { patterns: linesToList(repoPatternsEl.value) };
      ...
      try {
        ...
        const body = await res.json();
        const cfg = body && body.config ? body.config : payload;
        // Reflect the sanitized config (dedupe / trim) back into the input.
        repoPatternsEl.value = Array.isArray(cfg.patterns) ? cfg.patterns.join('\\n') : '';
        setRepoStatus('Saved');
        // Re-query so the visible tabs reflect the new filter immediately.
        loadCopilot();
        loadAll(true);
      } catch (e) {
        console.error('failed to save repo filter config', e);
        setRepoStatus('Save failed');
      } finally {
        repoSaveBtn.disabled = false;
      }
    }
    repoSaveBtn.addEventListener('click', saveRepoFilter);
    loadRepoFilter();
