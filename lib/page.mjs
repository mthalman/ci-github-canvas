  ...
  <script>
    ...
    function renderAzdo(azdo) {
      ...
      const buildLines = azdo.builds.map(b => {
        const ghJobs = sortJobsByFailureThenName(b.runs, runDotClass).map(r => \`<li><span class="ci-dot \${runDotClass(r)}"></span><a href="\${esc(r.detailsUrl)}" target="_blank" rel="noopener">\${esc(r.name)}</a> <span class="label">\${esc(runStatusLabel(r))}</span></li>\`).join('');
        ...
        if (!b.buildId) {
          ...
          return \`<div class="azdo-build">
            <div class="azdo-line azdo-flush"><span class="caret-spacer"></span><a href="\${esc(linkUrl)}" target="_blank" rel="noopener">\${esc(azdoBuildName(b))}</a> \${statusInline}</div>
          </div>\`;
        }
        const label = \`<a href="\${esc(b.summaryUrl)}" target="_blank" rel="noopener">\${esc(azdoBuildName(b))}</a>\`;
    ...
    function renderGha(gha) {
      ...
      const wfLines = workflows.map(w => {
        const jobs = sortJobsByFailureThenName(w.runs, runDotClass).map(r => \`<li><span class="ci-dot \${runDotClass(r)}"></span><a href="\${esc(r.detailsUrl)}" target="_blank" rel="noopener">\${esc(r.name)}</a> <span class="label">\${esc(runStatusLabel(r))}</span></li>\`).join('');
        if (!w.isWorkflow) {
          const r = w.runs[0];
          const statusInline = r
            ? \`<span class="ci-dot \${runDotClass(r)}"></span><span class="label">\${esc(runStatusLabel(r))}</span>\`
          ...
          return \`<div class="azdo-build">
            <div class="azdo-line azdo-flush"><span class="caret-spacer"></span><a href="\${esc(linkUrl)}" target="_blank" rel="noopener">\${esc(w.name)}</a> \${statusInline}</div>
          </div>\`;
        }
        const label = w.url
