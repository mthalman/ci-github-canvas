...
// Per-instance display snapshots: the last-rendered panel HTML the canvas page
// POSTs back so that returning to it after its webview was torn down and
// recreated on a brand-new ephemeral port still paints prior content instead of
// empty "Loading…" placeholders. Pure display cache (keyed by the stable canvas
// instanceId) — safe to delete, so it lives in its own subdir rather than in
// the unified settings document.
export const SNAPSHOTS_DIR = join(ARTIFACTS_DIR, "snapshots");
...
// Azure DevOps' Azure AD application (resource) ID. Passing it to
// `az account get-access-token --resource` mints a token scoped to AzDO REST
// APIs, which the ad-hoc CI-run feature uses to read private pipelines. This
// GUID is a well-known, public Microsoft constant (not a secret).
...
// All user-editable settings (notify + repo filter) live in a single unified
// document so the user has one place to inspect/edit them. lib/settings.mjs
// owns this file; each domain module owns the shape of its own section. The
// watched-PR list is intentionally NOT here — it's an append-heavy data
// collection, not configuration, and keeps its own watched-prs.json.
export const SETTINGS_PATH = join(ARTIFACTS_DIR, "settings.json");
// Legacy standalone config files, kept only as one-time migration sources for
// users upgrading from before the settings.json consolidation. settings.mjs
// folds these into the unified document on first load; nothing writes them.
export const LEGACY_NOTIFY_CONFIG_PATH = join(ARTIFACTS_DIR, "ci-runs.json");
export const LEGACY_REPO_FILTER_CONFIG_PATH = join(ARTIFACTS_DIR, "repo-filter.json");
